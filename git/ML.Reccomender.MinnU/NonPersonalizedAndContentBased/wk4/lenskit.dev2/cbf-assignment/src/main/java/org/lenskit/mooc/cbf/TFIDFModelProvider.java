    ...
    public TFIDFModel get() {
        ...
        for (long item : items) {
            ...
            for (Entity tagApplication : dao.query(TagData.ITEM_TAG_TYPE)
                ...
                //count it in the document frequency vector when needed
                if(work.get(tag) == 1.0) { //1st time I see the tag in the doc
        ...
        for (Map.Entry<Long, Map<String, Double>> entry : itemVectors.entrySet()) {
            ...
            // TConvert this vector to a TF-IDF vector
            ...
            for (Map.Entry<String, Double> e : tv.entrySet()) {
                e.setValue(e.getValue() / itemL2Norm);
            }
